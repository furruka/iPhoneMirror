...
##### 仍未修完：反控开着时画面会在几百帧后冻住

同步上下文那条修复是**真的有效但不完整**：卡死的位置从第 3 帧推到了第 800 帧左右
（星翼实测停在 821）。进程活着、`Sl` 状态、约 11% CPU、28 线程——**是阻塞而不是打转**。

到这里我已经连续两次给出「听起来合理但错」的解释，所以不再猜第三个。**已确立的事实**：

- 单独采集稳定（此前连续 5340 帧无问题）。
- 采集 + 反控同时开会冻住；同步上下文修复把冻住点从 3 帧推到约 800 帧。
- 冻住时进程阻塞，不是 CPU 打满。
- 这一次与鼠标交互相关（星翼当时正在试鼠标）。

**未确立**：具体是什么在阻塞。

**架构上正确的下一步是 `AcquireNotify`**（BlueZ ≥ 5.46）：它返回一个文件描述符，报告直接
写进去，**把 D-Bus 信号从每份报告的热路径上彻底移除**。以每秒上百份报告的频率去发
PropertiesChanged 信号本来就是错误的机制——**不管卡死是不是这个原因，这一步都该做**。
现在的实现之所以先用 PropertiesChanged，是因为它是 BlueZ 文档里最直白的那条路径。

在那之前，反控改成**显式开启**（`IPHONEMIRROR_REVERSE_CONTROL=1`），默认关。理由很简单：
镜像本身是稳的，把输入接上会让画面几百帧后冻住，**默认打开等于用一个能用的功能换一个坏掉
的功能**。
...
### P5b 前端移植：勘查与分期（已量清，尚未开工）

`src/App` 一共 **35,636 行**（`.cs` + `.xaml`）。按能不能直接用切一刀：

| 类别 | 数量 | 处置 |
|---|---|---|
| 平台中性 `.cs`（无 `using System.Windows` / `using Windows.`） | 48 个文件、**9,538 行** | **链接编译，不复制** |
| 依赖 WPF / WinRT 的 `.cs` | **34 个文件** | 需要 Avalonia 对应实现 |
| XAML 窗口 | **17 个** + `MainWindow.xaml` | 需要重写 |

中性文件的分布很有利：**37 个在 `Services/`**、6 个在 `Updater/`、3 个在 `Models/`——
也就是**业务逻辑基本是中性的，Windows 依赖集中在 UI 层**。这和 Bluetooth 那批文件的情况
一致（那 5 个零 `using` 的文件已经在链接编译）。

一个必须提前知道的约束：`iPhoneMirror.App.csproj` 依赖 **`WPF-UI 4.3.0`**（Fluent 风格的
WPF 控件库），**Avalonia 没有对等物**。所以 17 个窗口不是「换个 XAML 命名空间」就能过，
配色、控件外观、窗口装饰都得重新做一遍。这是这次移植真正的成本所在，不是那 34 个 `.cs`。

#### 分期

1. **把 48 个中性文件链接进来并编译通过。** 这一步会立刻暴露「自称中性其实不中性」的文件
   （比如 `BluetoothClientInfo.cs` 那种引了 localization 的），成本低、收益是把共享地基
   钉死。
2. **本地化。** `Localization/` 是 3 个 `.xaml` 资源字典 + 1 个 `.cs`，2,386 行。Avalonia 的
   资源查找方式不同，这是所有窗口的前置依赖，必须先做。
3. **主窗口。** 设备列表、预览、状态——已经有可用的预览控件，接的是同一套 C ABI。
4. **17 个子窗口**，按用户路径排序而不是按字母：设置类 > 通知类 > 开发者工具类。
5. **反控回归**（星翼指定在前端之后）：`AcquireNotify` 把 D-Bus 信号从报告热路径抽掉。

#### 为什么先勘查再开工

35k 行的移植如果边写边摸，会在「这个文件到底能不能直接用」上反复返工。**先把 48/34/17
这三个数字量出来，分期就有依据**——尤其是 `WPF-UI` 没有对等物这一条，如果开工两天后才发现，
前面按「换命名空间」思路写的窗口全要推翻。
